...
const CONFIG = {
  /*URL: '.', //LOCAL*/
  URL: '/content/Componentes/NoticiasUPBVIRTUAL', //PRODUCCIÓN
  data: '/news.json',
...
async function loadData() {
  try {
    ...
    const response = await fetch(CONFIG.URL + CONFIG.data);
    console.log('Cargando noticias desde:', response);
...
function showError(containerId) {
  ...
  if (el) el.innerHTML = '<p class="error">Lo sentimos, no se pudo cargar la información.</p>';
}
...
function processData(data) {
  ...
  const eventsData = data
    ...
    .sort((a, b) => {
      const da = parseDateDDMMYYYY(a.fechacorte || a.date);
      const db = parseDateDDMMYYYY(b.fechacorte || b.date);
      return db - da; // ordenar descendente por fecha de evento
    });
  ...
  if (items.length === 0) {
    container.innerHTML = `
      <div class="empty-state">
       <img src="${CONFIG.URL.replace(/\/$/, '')}/images/ProximosEventos.png" 
...
function createCard(item) {
  ...
  article.innerHTML = `
    <img src="${CONFIG.URL}${item.image}" title="${item.titular}" alt="${item.titular}" loading="lazy" />
...
// Limpieza de fecha para atributo datetime en formato ISO (YYYY-MM-DD)
